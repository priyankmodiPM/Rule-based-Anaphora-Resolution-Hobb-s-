...
def case_match(tree, pos, pro):
    """ Takes a proposed antecedent and pronoun and checks whether 
    they match in case.
    """
    nominal = ["वह", "वो"]
    ergative = ["इसने", "इन्होने"]
    accussative = ["उसे", "उसको", "उन्हें", "उनको"]
    instrumental = ["उससे", "उसके द्वारा", "उनसे", "उनके द्वारा"]
    dative = ["उसको", "उसे", "उसके लिए", "उनको", "उन्हें", "उनके लिए"]
    ablative = ["उससे", "उनसे"]
    genative = ["उसका", "उसकी", "उसके", "उनका", "उनकी", "उनके"]
    locative = ["उसमें", "उसपर", "उनमें", "उनपर"]
 
    for c in tree[pos]:
        if isinstance(c, nltk.Tree) and c.label() in nominal_labels:
            if c.leaves()[0].lower() in nominal:
                if pro not in nominal:
                    return False
            if c.leaves()[0].lower() in ergative:
                if pro not in ergative:
                    return False

            if c.leaves()[0].lower() in accussative:
                if pro not in accussative:
                    return False
            
            if c.leaves()[0].lower() in instrumental:
                if pro not in instrumental:
                    return False

            if c.leaves()[0].lower() in dative:
                if pro not in dative:
                    return False

            if c.leaves()[0].lower() in ablative:
                if pro not in ablative:
                    return False

            if c.leaves()[0].lower() in genative:
                if pro not in genative:
                    return False

            if c.leaves()[0].lower() in locative:
                if pro not in locative:
                    return False
    return True
...
def gender_match(tree, pos, pro):
    ...
    male_pronouns = ["he", "him", "himself", "वह", "वो", "इसने"]
    female_pronouns = ["she", "her", "herself", "वह","वो", "इसने"]
    neuter_pronouns = ["it", "itself", "वह", "वो", "इसने"]
    ...
    if len(sys.argv) == 2 and argv[1] == "demo":
        ...
        if len(sys.argv) > 3 or len(sys.argv) < 2:
            ...
            p = ["He", "he", "Him", "him", "She", "she", "Her",
                "her", "It", "it", "They", "they", "वह", "वो", 
                "इसने", "इन्होने", "उस", "उसको", "यह", "उनको", "उनका",
                 "उनकी", "उनसे", "उनके" ]

            r = ["Himself", "himself", "Herself", "herself",
                "Itself", "itself", "Themselves", "themselves",
                "अपने", "अपनेआप", "अपना", "अपनी"]
